...
bool UInventoryWidget::NativeOnDrop(const FGeometry& InGeometry, const FDragDropEvent& InDragDropEvent, UDragDropOperation* InOperation)
{
    ...
    // Eşya taşınamazsa, envanterde ilk bulunduğu yerde kalmasını sağla
    if (InventorySubsystem && ItemObject)
    {
        bool bIsSuccess;
        InventorySubsystem->TryAddItem(ItemObject, bIsSuccess);
        if (!bIsSuccess)
        {
            UE_LOG(LogTemp, Warning, TEXT("Item could not be re-added to the inventory."));
        }
    }
